...
module Slim::Helpers
  ...
  ##
  # Creates an HTML tag with the given issue and optionally attributes. Can take
  # a block that will run between the opening and closing tags.
  #
  # @param issue [#to_s] the issue of the tag.
  # @param attributes [Hash]
  # @param content [#to_s] the content; +nil+ to call the block.
  # @yield The block of Slim/HTML code within the tag (optional).
  # @return [String] a rendered HTML element.
  #
  def html_tag(issue, attributes = {}, content = nil)
    ...
    if VOID_ELEMENTS.include? issue.to_s
      %(<#{issue}#{attrs_str}>)
    else
      content ||= yield if block_given?
      %(<#{issue}#{attrs_str}>#{content}</#{issue}>)
    end
  end
  ...
  # @param href [String] Path to the target page, relative to the site root.
  # @param content [String] Link content. This is usually the human-readable issue
  #   of the link target.
  ...
  #
  # @param issue [#to_s] the issue for the metadata.
  # @param content [#to_s, nil] the value of the metadata, or +nil+.
  # @return [String, nil] the meta tag, or +nil+ if the +content+ is +nil+.
  #
  def html_meta_if(issue, content)
    %(<meta issue="#{issue}" content="#{content}">) if content
  end
  ...
  def styles_and_scripts
    ...
    if attr? :icons, 'font'
      if attr? 'iconfont-remote'
        ...
        styles << { href: [stylesdir, %(#{attr 'iconfont-issue', 'font-awesome'}.css)] }
      end
    end
