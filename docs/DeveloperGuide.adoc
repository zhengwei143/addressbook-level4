...
In the above figure, the left column is the list panel which displays the list of issues or
the list of solutions. The column in the middle displays solutions of the selected issue. The blank part
...
** Analyses the argument to check if it is a valid command format. Users are only allowed to use two prefixes in refactor tag command.
*** `t/` the tag that user wants to refactor, which must be input by the user, otherwise throws `ParserException` and shows refactor tag command usage message.
...
* Alternative 1 (current choice):** Allows range of `Index` to be accepted as index parameter.
...
==== Current Implementation
The Command Highlight mechanism uses `InlineCssTextArea` class. It uses listener to detect the change in the `commandTextField` and check if it is command word, parameter, index or values and assign different colors correspondingly.
