...
In order to allow sorting to work with filtered list, we wrap `FilteredList` with `SortedList` and retrieve this new list with a new method `getFilteredAndSortedList()`.
...
They can serve the three sort types (excluding the default one) we provide in SaveIt. When more sort types are needed, we can simply create another class implementing Comparator, and add anther case in switch statement.
...
If it is primary, we replace it by a new Solution object with the same data of the primary solution. If it is not primary, we continue checking until we reach the end of the list.
// end::resetprimary[]
