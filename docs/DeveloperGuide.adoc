...
In order to allow sort to work with filtered list, we wrap `FilteredList` with `SortedList` and retrieve this new list with a new method `getFilteredAndSortedList()`.
...
As shown in the diagram, instead of letting LogicManager execute the command directly after parsing it, we now check if we need confirmation before the command is executed.
...
The reset primary solution mechanism goes through the solution list once and check if a solution is primary or not.
If it is primary, then replace it by a new Solution object with the same fields. If it is not primary, we continue checking until reach the end of the list.
// end::resetprimary[]
