...
* `ModelManager#updateFilteredIssueList(Predicate<Issue>)` -- Predicate is set to be unused. Indicate no change is required.
* `FXCollections.unmodifiableObservableList(FilteredList<Issue>)` -- Return a ObservableList of all issues.
...
Step 2. The user executes `delete 5` command to delete the 5th statement in the address book. The `delete` command calls `Model#commitSaveIt()`, causing the modified state of the address book after the `delete 5` command executes to be saved in the `saveItStateList`, and the `currentStatePointer` is shifted to the newly inserted address book state.

image::UndoRedoNewCommand1StateListDiagram.png[width="800"]

Step 3. The user executes `add n/David ...` to add a new statement. The `add` command also calls `Model#commitSaveIt()`, causing another modified address book state to be saved into the `saveItStateList`.
...
Let's add a placeholder on all our link:{repoURL}/src/main/java/seedu/address/ui/PersonCard.java[`PersonCard`] s to display a remark for each statement later.
...
. Fix `invalidAndValidPersonSaveIt.xml`, `typicalIssuesSaveIt.xml`, `validSaveIt.xml` etc., such that the XML tests will not fail due to a missing `<remark>` element.
...
|`* * *` |programmer |record the statement and email |review it when i encountered it again
...
|`* *` |a programmer dealing with many programs at the same time |search the statement based on the tag or key words |So that i can find the statement easily

|`* *` |a programmer encounter various issues while coding |add the tag for each statement |collect same tag

|`* *` |a CS student who aims to improve technical skills |store the way I solved certain statement |review it and see if there is a better email in the future
...
2. SaveIt add the issue to its storage
3. The ‘Success’ message shown on the screen.
+
Use case ends.
...
+
[none]
** 2a1. The 'Invalid Command' message shown on the screen.
+
...
1. User request to list issues
2. SaveIt shows a list of issues
3. User select a specific issue in the list
...
7. The 'Success' message shown on the screen.
+
Use case ends.
...
[discrete]
=== Use case: Delete statement

*MSS*

1.  User requests to list issues
2.  SaveIt shows a list of issues
3.  User requests to delete a specific statement in the list
4.  SaveIt deletes the statement
5. The "Success" message shown on the screen.
+
Use case ends.
