...
* Parameters can be in any sort_type e.g. if the command specifies `t/TAG, d/DESCRIPTION i/ISSUE_STATEMENT` is also acceptable.
====
...
* The sort_type of the keywords does not matter. e.g. ‘kill port’ will match ‘port kill’
...
[NOTE]
====
* All issues are listed in chronological sort_type by default in home directory.
====
...
Lists all the commands that you have entered in reverse chronological sort_type.
...
Shows a list of all issues in the statement bookmark according to either chronological sort_type or frequency sort_type. Users can also choose to list part of issues.

Format: `list chr`

[TIP]
====
* List all the issues by chronological sort_type.
====
...
[TIP]
====
* List all the issues in alphabetical sort_type.
