...
To add an issue to the SaveIt App.
...
* The url validation check only provides most basic format checking
====
...
To delete the specified issue from SaveIt App.
...
To find issues whose statement contain any of the given search queries.
...
[NOTE]
====
* List command can only be executed in root level.
====
