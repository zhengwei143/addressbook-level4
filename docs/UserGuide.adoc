...
.Edits solution link and remark of the second solution
...
* The order of the issues may be updated if the user uses sort command.
* This command can only be executed in root directory.
====
// end::addtag[]
...
[NOTE]
====
* User needs to confirm this command before the operation is executed.
...
[TIP]
====
* Sorts the issue list based on the time when they are created. The first issue added has the highest priority.
====

Format: `sort chro`
[TIP]
====
* Sorts the issue list by their last modified time chronologically. The last modified one has the highest priority.
...
[TIP]
====
* Sorts the issue list based on search frequency. The one which matches the search keywords most frequently has the highest priority.
====

Format: `sort tag`
[TIP]
====
* Sorts the issue list by each issue's tag set in lexicographical order, following ASCII lexicography.
