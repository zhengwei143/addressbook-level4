...
=== Add tag Feature
The add tag feature allows the user to add a tag or more tags to a specified issue.
...
* **Alternative 1 (current choice):** Adds tag(s) for one issue in command execution.
** Pros: It allows multiple additions of `Tags` in one command execution.
** Cons: It has a lower efficiency to add tags for multiple issues.
* **Alternative 2:** Creates a separate class for ranges of index
** Pros: It is convenient for users to add tags for multiple issues once.
