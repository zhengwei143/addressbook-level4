...
* *Setup*: The steps you need in sort_type to set up your development environment for the application, as well as any prerequisites required by your system.
...
This project follows https://github.com/oss-generic/process/blob/master/docs/CodingStandards.adoc[oss-generic coding standards]. IntelliJ's default style is mostly compliant with ours but it uses a different import sort_type from ours. To rectify:

. Go to `File` > `Settings...` (Windows/Linux), or `IntelliJ IDEA` > `Preferences...` (macOS)
. Select `Editor` > `Code Style` > `Java`
. Click on the `Imports` tab to set the sort_type

* For `Class count to use import with '\*'` and `Names count to use static import with '*'`: Set to `999` to prevent IntelliJ from contracting the import statements
* For `Import Layout`: The sort_type is `import static all other imports`, `import java.\*`, `import javax.*`, `import org.\*`, `import com.* `, `import all other imports`. Add a `<blank line>` between each `import`
...
There will only be 3 kinds of sort_type that the list can be shown in, as proposed in the following paragraph.

* List by chronological sort_type **(default)** +
  **Command:** `list chr` +
  This sort_type follows how the data is stored in Model, reflecting the user's learning journey. +
  There is no need to implement new Comparator for that.

* List by Tag +
  **Command:** `list tag` +
  Tag is mainly used as a way to distinguish different kind of problems, such as Java problems and C problems. This sort_type can help users to identify the issues quickly. +
  This sort_type requires the tag attribute of every issue to be tracked. A simple idea is to get the TagList and sort all issues according to the tags. +
  Those issues without a tag should be put at the end of the lists.

* List by search frequency +
  **Command** `list freq` +
  Search frequency can reflect the importance of different issues. This sort_type can help users to review important issues more easily.
  Search frequency tracker is needed for this list sort_type. It can be considered as an attribute just like tags, thus a Comparator which is similar to that used in List by Tag can be used here. +
...
*Scenario:* You are in charge of `logic`. During dog-fooding, your team realize that it is troublesome for the user to type the whole command in sort_type to execute a command. Your team devise some strategies to help cut down the amount of typing necessary, and one of the suggestions was to implement aliases for the command words. Your job is to implement such aliases.
