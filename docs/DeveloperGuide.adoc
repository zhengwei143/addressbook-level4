...
As shown in the diagram, an IssueSort object contains a Comparator object as its attribute, and the type of that object depends on the input String passed from SortCommand.
We consider switch statement as a proper way to provide alternative paths to initialize Comparator here for current implementation.
