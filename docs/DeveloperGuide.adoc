...
** Analyses the argument to check if it is a valid command format. If it is a valid format, it will set the corresponding fileds usign `EditDescriptor`. There are two types format.
...
** Analyses the user in the root directory or issue directory. If the user in the root directory but provide solution level edit command or in solution directory but provide issue level edit command +
...
image::EditCommandSequenceDiagram.jpg[width="800"]
...
The refactor tag feature allows users to rename or remove a specific tag for all entries with that tag.
...
** update the issue list and return success message if it has edited an issue, otherwise show unsuccessful message.
