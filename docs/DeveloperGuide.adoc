...
** Analyses the user in the root directory or issue directory. Since the root directoy can only edit issues, if user provides solution related command, it will fail to execute. Also, in the issue directory, it only allows to edit solution, it will fail to execute the issue related command. Then it will
...
* **Alternative 1 (current choice):** Allows range of indexes and multiple indexes to be accepted as index parameter.
