...
In order to allow sort work with filtered list, we wrap `FilteredList` with `SortedList` and retrieve this new list with a new method `getFilteredAndSortedList()`.

The following two sequence diagrams illustrate how the mechanism works:
...
This two diagrams show that after the input is parsed, a SortType object is initialized before the SortCommand object.

As shown in the diagram, an SortType object can provide the required comparator with the input String passed from SortCommand.
We use the retrieved Comparator to sort the list when the sort command is executed.

The following is a class diagram for SortType.

.SortType Class Diagram
image::SortType.png[width="800"]
...
They can serve the three sort types we provide in SaveIt. When more sort types are needed, we can simply create another class implementing Comparator, and add anther case in switch statement.
...
=== UI Enhancement
The figure below shows the current UI for SaveIt v1.4.
...
==== Current Implementation
The edit mechanism is facilitated by `editCommandParser` and `editCommand`. Users are allowed to edit any field in the issue list. includes: +
...
** Analyses the user in the root directory or issue directory. If the user in the root directory but provide solution edit command or in issue level but provide issue edit command +
...
Please refer to the Sequence Diagram below for the edit operation.
...
** Cons: It has lower security if it the issue is the mutable object.
// end::edit[]
...
// tag::setprimary[]
=== Set Primary feature
The set primary feature allows users to choose one solution as primary solution.

==== Current Implementation

The set primary mechanism is facilitated by the method `isPrimarySolution()` override by `PrimarySolution` inheriting from `Solution`.
...
** Pros: No flag attribute is introduced. Solution structure remains the same.
...
* **Alternative 3:** Store the primary solution index in solution list.
...
*** `t/` the tag that user wants to refactor, which must be input by user, otherwise throws `ParserException` and shows refactor tag command usage message.
*** `n/` the new tag that user wants to replace the original one, which is optional. (if the user does not provide, then remove the original one)
...
** update the issue list and return success message if it has edited an issue, otherwise show an unsuccessful message.
** It will iterate all the issues in the saveIt, if it contains the old tag, it will replace with the new tag if provided. Then update the list.

Please refer to the Sequence Diagram below for the refactor tag operation.

.Refactor Tag Command Activity Diagram
image::refactor-tag.jpeg[width="800"]
...
** Cons: It is inconvenient if the user wants to replace the tags once.
...
Please refer to the Sequence Diagram below for the add tag operation.
...
* **Alternative 1 (current choice):** Allows range of `Index` and discrete `Index` to be accepted as index parameter.
...
* Alternative 2: Creates a IndexRange class for index.
** Pros: It is easy to understand and maintain.
** Cons: It will increase coupling between IndexRange class and Index class
// end::addtag[]
...
==== Current Implementation
The Command Highlight mechanism uses `InlineCssTextArea` class. It uses listener to detect the change in the `commandTextArea` and check if it is command word, parameter, index or values and assign different colors correspondingly.

==== Design Consideration
===== Aspect: Implementation of command highlight manager.

* **Alternative 1 (current choice):** Check the position of user input in the command box and assign the color
** Pros: easy to implement by check the prefix, index and command word.
** Cons: due to the limitation of the solution link, if the link contains the prefix, it will be highlighted.
* **Alternative 2:** Parse the user command simutaneouly when user input.
** Pros: if user input wrong format, it will highlight the color corresponging.
